...
data = []            # Data zarejestrowania danych
...
przeplyw = przeplyw[::-1]
temp_zasilania = temp_zasilania[::-1]
temp_powrotu = temp_powrotu[::-1]
roznica_temp = roznica_temp[::-1]
moc = moc[::-1]
...
for nazwa,zmienna in zmienne_do_naprawienia.items():
    for index,wartosc in enumerate(zmienna): # Iterujemy po wszystkich obserwacjach
        ...
        if (wartosc > 10000):
            ...
            # Dodajemy wartosc do słownika
           #zmienne_do_naprawienia.append()
...
with open("predykcja.json", "w") as outfile:
